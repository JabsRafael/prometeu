...
import { treeMenu, type Context, type Entry, type Hooks } from "./tree-menu";
...
function hooks(): Hooks {
  return { open: vi.fn(), toggle: vi.fn(), attach: vi.fn(), copy: vi.fn(), reveal: vi.fn() };
}

function context(over: Partial<Context> = {}): Context {
  return { root: ROOT, expanded: false, hooks: hooks(), ...over };
}
...
describe("file menu", () => {
  it("offers opening, attaching and the path actions in that order", () => {
