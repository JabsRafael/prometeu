...
| create, rename and trash from the side file tree | independent of the CLI | independent of the CLI | shared application behavior | `session/files.rs` tests for names, conflicts and symlinks; checked in the browser mock, not covered in E2E; trashing uses the system trash and is not exercised in tests |
