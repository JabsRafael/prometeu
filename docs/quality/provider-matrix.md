...
| create, rename, trash and restore from the side file tree | independent of the CLI | independent of the CLI | shared application behavior | `session/files.rs` tests for names, conflicts and symlinks, `restore_deleted` tests in `session/git_tests.rs`; checked in the browser mock, not covered in E2E; trashing uses the system trash and is not exercised in tests |
