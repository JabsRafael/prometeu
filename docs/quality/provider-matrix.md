...
| create, rename, trash and restore from the side file tree | independent of the CLI | independent of the CLI | shared application behavior | `src/tree-menu.test.ts` for the menus and for tabs, drafts and folders following a moved entry; `session/files.rs` tests for names, conflicts and symlinks; `restore_deleted` tests in `session/git_tests.rs`; checked by hand in the browser mock, not covered in E2E; the system trash itself is not exercised in tests |
