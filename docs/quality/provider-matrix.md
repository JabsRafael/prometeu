...
| Capability | Claude | Codex | Gemini | Main evidence |
| --- | --- | --- | --- | --- |
...
| the organization's plugins, MCPs and skills | explicit local installation in the hub | the same UI and installation; existing adapters | To confirm | `catalog.rs`, `e2e/cloud.spec.ts`, `organizations_test.rb`; it requires no personal copy and does not activate automatically |
...
| Code review included and editable | the initial profile; the model/provider can be changed | can be chosen in the profile | To confirm | `actions.rs`, `actions.test.ts`, `e2e/actions.spec.ts` |
| prompt commands and tasks | adapted by the app | adapted by the app | To confirm | `actions.test.ts`, `e2e/actions.spec.ts` |
| per-task profile | instructions and permissions through flags | instructions and permissions through JSON-RPC | To confirm | `session.rs` and `codex.rs` tests |
| PR tracking | local polling by the app | local polling by the app | To confirm | `actions.rs`, `github.rs`; no real GitHub integration in tests |
| detecting the installation | adapted | adapted | version checked (minimum 0.30.0) | `agents.rs` |
| accounts and global selection in the footer | adapted through `CLAUDE_CONFIG_DIR` | adapted through `CODEX_HOME` | To confirm | `accounts.rs`, `e2e/accounts.spec.ts` |
| removing every account and an empty selection | supported; preserves the CLI's login | supported; preserves the CLI's login | To confirm | `accounts.rs`, `e2e/accounts.spec.ts`; directories and credentials stay local |
| login through the app | the CLI's `auth login` and the browser | the app-server's `account/login/start` and the browser | Google / API key; real OAuth to confirm | identity fixtures in `claude.rs` and `codex/account.rs`; OAuth with two real accounts still requires manual validation |
| switching accounts between turns | resuming the shared transcript | resuming the shared rollout/index | To confirm | `chat.rs`, profile tests; an authenticated continuation between two real accounts is not yet proven by the suite |
| quotas per account | stream and internal endpoint | app-server and internal fallback | API key: no windows; Google: to confirm | `usage.rs`, `e2e/accounts.spec.ts` |
| live model catalog | native through a control request | native through the CLI's cache | CLI aliases; no live catalog in ACP 0.30.0 | `agents.rs` |
| starting a session | native | adapted to JSON-RPC | Adapted; authenticated CLI conformance to confirm | `chat.rs`, `codex.rs` |
| resuming a session | Claude's id/transcript | the app-server's thread | Adapted; authenticated CLI conformance to confirm | `session.rs`, Rust tests |
...
| tools in mixed-provider tabs and actions | CLI base follows the effective Claude provider and configured home | hub-only universe even in a Claude workspace | To confirm | `session.rs::tool_resolution_uses_the_tab_provider_and_configured_claude_home`, `e2e/tools.spec.ts` |
| tool reset and project trust | null restores inheritance; decisions bind to the displayed hash, including empty declarations | same contract | To confirm | `session.rs::tool_axis_ipc_preserves_absent_null_and_replacement`, `session.rs::projeto_so_injeta_depois_de_aprovado_e_reprova_quando_o_hash_muda`, `e2e/tools.spec.ts` |
| ordered prompt, transcript, and live delivery | shared conversation mutex | shared conversation mutex | To confirm | concurrent delivery, fast-response, and failed-write regressions in `chat.rs` |
| choosing a model | native through a flag | adapted in `thread/start`/`thread/resume` | Adapted; authenticated CLI conformance to confirm | `session.rs`, `codex.rs` |
| effort levels | catalog + fallback | Codex's catalog | Unavailable | `agents.rs`, `launcher.ts` |
| conversation footer adapted to the width | model and activity separated from the tools; remote control highlighted when active | the same UI | To confirm | `e2e/composer.spec.ts`, Chromium/WebKit, Portuguese/English and narrow frames |
...
| initial plan mode | native through permission mode | unavailable | Adapted; authenticated CLI conformance to confirm | `session.rs`, `launcher.ts` |
| streaming text | adapted to V1 | adapted to V1 | Adapted; authenticated CLI conformance to confirm | `conversation.test.ts`, `timeline.test.ts`, `codex.rs` tests |
| thinking | adapted to V1 | adapted to V1 | Adapted; authenticated CLI conformance to confirm | `timeline.test.ts`, `codex.rs` tests |
| tool call and result | adapted to V1 | adapted to V1 | Adapted; authenticated CLI conformance to confirm | `conversation.test.ts`, `claude.rs`/`codex.rs` tests |
| questions to the user | native | adapted from a JSON-RPC request | Unavailable | `chat.ts`, `codex.rs` tests |
| approval requests | native | adapted | Adapted; authenticated CLI conformance to confirm | `chat.rs`, `codex.rs` |
| interruption | control request | `turn/interrupt` | Adapted; authenticated CLI conformance to confirm | `codex.rs`, Rust tests |
...
| layered selection (global, project, workspace) per axis | resolved at spawn before the adapter | resolved at spawn before the adapter | To confirm | `selection.rs` resolve tests, `session.rs::provenance_classifica_cada_item_do_hub`, `session.rs::projeto_so_injeta_depois_de_aprovado_e_reprova_quando_o_hash_muda` and `session.rs::o_payload_do_eixo_e_validado_antes_de_gravar`; the project `[tools]` layer is gated on trust-on-first-use of its hash, an undecided item stays `pending` and a rejected one stays `rejected` (both resolved yet not injected), and the setters refuse a malformed payload or an id on the wrong axis |
...
| attachments in a message, capture thumbnails and pasting | adapted through a local path; promise and pasteboard materialized by macOS | adapted through a local path; promise and pasteboard materialized by macOS | Unavailable | `file_drop.rs`, `chat.ts`, `paste.ts`; `e2e/file-drop.spec.ts` and dropped-file scenarios in `e2e/critical-flows.spec.ts` cover the UI over the mock; `paste.test.ts` covers the paste detour |
| the browser's visual context | a tag in the draft and the history; complete HTML, CSS, URL and PNG mention on send | the same interface and textual contract | To confirm | `browser-context.test.ts`, `e2e/browser-inspector.spec.ts`, `e2e/browser.spec.ts`, `browser.rs` tests; WKWebView capture and the AppKit gesture still require native verification |
| unknown external event | ignored by the adapter | ignored by the adapter | Adapted; authenticated CLI conformance to confirm | `conversation.test.ts`, `claude.rs`/`codex.rs` tests |
| the CLI's subagents | a sidechain off-screen; tasks in `background.changed` | `collabAgentToolCall.agentsStates`, `subAgentActivity` and known child events update the tasks; the children's content stays isolated | To confirm | `claude.rs`; isolation, spawn, activity and partial-state tests in `codex.rs` |
| attention indicators without audio | completion, question and comment pending items in the Dock | the same rule over live local V1 events | To confirm | `src/alert.test.ts` preserves counting and reading; `e2e/alerts.spec.ts` in Chromium/WebKit covers the absence of audio and of the sound option |
...
| agents per workspace in the sidebar | each tab's brand and status | each tab's brand and status | To confirm | sidebar flows in `e2e/critical-flows.spec.ts`; a remote one uses the owner's avatar, without inferring the provider |
...
## Gemini verification boundary

The integration targets the installed Gemini CLI 0.30.0. Its ACP handshake was
observed locally, but `session/new` returned authentication-required (401) in the
available environment. Protocol examples derived from the installed source are
not recordings of successful authenticated turns. Automated account UI evidence
is in `e2e/accounts.spec.ts`; adapter/profile evidence is in `gemini.rs` and
`gemini/account.rs`. A real Google login, key-backed prompt, tool approval,
cancellation and killed-process/cross-account resume remain manual smoke tests.
Do not close the conformance gap merely because mocks pass.

The source-derived `src-tauri/src/gemini/fixtures/canonical-events.json` is
