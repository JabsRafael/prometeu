...
| image pasted from the clipboard | `<root>/attachments/<uuid>/pasted.png` | `file_drop.rs`; same folder and permissions, TIFF converted to PNG; outside macOS the webview image keeps its type as `pasted.{png,jpg,gif,webp}` |
