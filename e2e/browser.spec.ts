...
test("browser mantém preview ao trocar conversa e adiciona contexto somente ao destino atual", async ({ page }) => {
  await openBrowser(page);
  const composer = page.locator(composerSelector);
  await composer.fill("Rascunho da primeira conversa.");
  await pickElement(page);
  await page.locator('#tabbar .tab[data-tab="t2"]').click();
  await expect(page.locator(previewSelector)).toBeVisible();
  await expect(page.locator("#webdetails")).toBeVisible();
  await composer.fill("Rascunho da segunda conversa.");
  await page.locator("#wadd").click();
  await expect(composer).toHaveValue("Rascunho da segunda conversa.");
  await expect(page.locator(contextSelector)).toHaveCount(1);
  await expect(page.locator(fileSelector)).toHaveCount(0);
  await page.locator('#tabbar .tab[data-tab="t1"]').click();
  await expect(composer).toHaveValue("Rascunho da primeira conversa.");
...
test("browser preserva tag e captura na fila pendente sem expor JSON após falha de retomada", async ({ page }) => {
  await openBrowser(page);
  const composer = page.locator(composerSelector);
  await composer.fill("Mensagem para sessão parada.");
  await pickElement(page);
  await page.locator("#wadd").click();
  await page.evaluate(() => {
    const target = window as BrowserTestWindow;
    ...
    target.__TAURI_INTERNALS__.invoke = async (command, args) => {
      ...
      throw "Falha simulada depois de enfileirar.";
    };
  });
  await page.locator("#chatwrap .send").click();
  await expect.poll(() => page.evaluate(() => (window as BrowserTestWindow).browserMessages.length)).toBe(1);
  ...
  await expect(pending).toContainText("Mensagem para sessão parada.");
  await expect(pending.locator(".browser-context")).toContainText("Elemento selecionado");
...
test("browser entrega captura atrasada à conversa original depois de trocar aba", async ({ page }) => {
  await openBrowser(page);
  const composer = page.locator(composerSelector);
  await composer.fill("Captura para a primeira conversa.");
  await hold(page, "browser_capture");
  await page.locator("#wcapture").click();
  await waiting(page);
  await expect(page.locator("#chatwrap .send")).toBeDisabled();
  await page.locator('#tabbar .tab[data-tab="t2"]').click();
  await composer.fill("Outra conversa.");
  await release(page);
  await expect(page.locator("#wcapture")).toBeEnabled();
  await expect(page.locator("#chatwrap .cfiles .injchip")).toHaveCount(0);
  await expect(composer).toHaveValue("Outra conversa.");
  await page.locator('#tabbar .tab[data-tab="t1"]').click();
  await expect(page.locator("#chatwrap .cfiles .injchip")).toContainText(/browser-.*\.png/);
  await expect(composer).toHaveValue("Captura para a primeira conversa.");
  await expect(page.locator("#chatwrap .send")).toBeEnabled();
  expect(await page.evaluate(() => (window as BrowserTestWindow).browserSent)).toBe(0);
});

test("browser descarta seleção redimensionada durante captura e permite inspecionar novamente", async ({ page }) => {
  await openBrowser(page);
  await hold(page, "browser_capture");
  await page.locator("#winspect").click();
  await expect(page.locator("#winspect")).toHaveAttribute("aria-pressed", "true");
...
test("browser preserva contexto textual quando captura falha", async ({ page }) => {
  await openBrowser(page);
  await page.evaluate(() => {
    const target = window as BrowserTestWindow;
    const original = target.__TAURI_INTERNALS__.invoke;
  ...
  await page.locator(composerSelector).fill("Minha referência.");
  await pickElement(page);
  await expect(page.locator("#webdetails .webmeta")).toContainText("Sem captura");
  await page.locator("#wadd").click();
  await expect(page.locator(composerSelector)).toHaveValue("Minha referência.");
  await expect(page.locator(contextSelector)).toContainText("Elemento selecionado");
  await expect(page.locator(fileSelector)).toHaveCount(0);
  expect(await page.evaluate(() => (window as BrowserTestWindow).browserSent)).toBe(0);
});
...
test("browser permite arrastar arquivo ao chat e fechar preview preserva conversa e rascunho", async ({ page }) => {
  ...
  await composer.evaluate(element => {
    ...
    target.mock.drop(["/Users/eu/Desktop/Referência.png"], rect.x + rect.width / 2, rect.y + rect.height / 2);
  });
  ...
  await expect(page.locator("#side")).toBeVisible();
  await expect(page.locator("#chatwrap")).toBeVisible();
  await expect(composer).toHaveValue("Use esta referência.");
  await expect(page.locator("#chatwrap .cfiles .injchip")).toContainText("Referência.png");
});
...
test("browser suspende para menus e diálogos, restaurando página sem recarregar", async ({ page }) => {
  ...
  await name.fill("Projeto preservado");
  ...
  await expect(name).toHaveValue("Projeto preservado");
  await page.evaluate(() => {
...
test("browser ignora abertura atrasada ao sair do workspace e não cobre outra conversa", async ({ page }) => {
