...
Prometeu is an open-source macOS app for working with Claude Code and Codex.
...
3. Install Claude Code, Codex, or both. The corresponding `claude` or `codex`
   command must be available in your shell.
...
- Claude Code or Codex installed and authenticated to exercise real sessions.
...
Local work does not require a Prometeu account. App state and Codex conversation
...
- [Provider matrix](docs/quality/provider-matrix.md): Claude Code and Codex support.
